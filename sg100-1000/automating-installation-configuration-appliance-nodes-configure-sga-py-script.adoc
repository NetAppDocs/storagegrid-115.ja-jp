...
. アプライアンスの設定をJSONファイルにバックアップする場合は、advanced権限とを使用します `backup-file` サブコマンド。たとえば、IPアドレスを持つアプライアンスの設定をバックアップする場合などです `_SGA-INSTALL-IP_` を指定します `appliance-SG1000.json`をクリックし、「+」と入力します
